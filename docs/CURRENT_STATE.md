...
_Last updated: 2026-05-31 — **Full UI/UX overhaul (audit → fix → validate → deploy).** A 13-agent parallel audit produced 200 findings → 12 themes → 68 deduped prioritized items; ALL severities (Critical/High/Medium/Low) were implemented across 7 commits (`95a0c94`→`83f5b72`). **Foundations** (`globals.css`): WCAG-AA token ramp (`--muted` #9a958c, `--muted-2` #928c80 — were 3.0–3.4:1 as text; `--rust-strong` AA white-on-rust button fill; `--red-text` AA red text; `*-rgb` tints), spacing/type/elevation/z scales, solid focus-visible ring (was 0.7-alpha, failed non-text contrast), `prefers-reduced-motion`, `.srOnly`/`.skipLink`. **Primitives** (`ui.tsx`, now `'use client'` + `UIProvider` in root layout): `Modal` (focus-trap/ESC/portal), `Toast`+`useToast` (aria-live), `Confirm/Prompt`+`useConfirm`/`usePrompt` (replaced ALL native `window.confirm/prompt/alert` across 7 files), `EmptyState`, `Banner`, extended `TextField` (error/hint/required + aria) / `Button` (secondary, danger-solid, aria-busy) / `Segmented` (roving tabindex + arrows). **Shell**: skip-link, `<main>` landmark, `aria-current` + non-color active nav, labeled nav, responsive hamburger drawer, dynamic per-route `document.title`, platform sub-nav → module + mobile scroller. **Screens**: real landing (value prop + dual CTA + authed→/dashboard, killed the "scaffolding" copy); auth validation/aria-live/password-reveal; overview new-buyer EmptyState CTA + error-Retry + keyboard drill-down + compact-$ + freshness; analytics sortable-header a11y + SearchSelect/Segmented filters + responsive sticky-col table; approvals Approve/auto-launch confirm + bulk reject reason; facebook token-expiry Badge + scopes + status; wizard stepper-a11y + collapsible cards + label-assoc + sticky footer + responsive; charts role=img + data-table + colorblind-safe series + axes; SearchSelect combobox ARIA + keyboard; DateRange grid keyboard nav; platform confirm/toasts + form labels + responsive admin tables; **article money-pages**: per-host header/byline/date/footer (thin-site fix), per-host brand on `/search` (was hardcoded "10 Lines About"), real home + not-found, **ads≤results** (SearchAds capped at / suppressed without the organic count), deterministic lead/body de-dup, light editorial theme + focus. The article `cid` channel param + CSA/monetization wiring were left intact (verified). **Gate**: web+article+shared all typecheck/lint/build green; 66 shared tests pass. **DEPLOYED web+article to staging** (`push-staging.sh web article`; dry-run zero deletions; api/worker untouched — only additive shared change). Live-verified: web `/`+`/login` 200 with new landing copy; article `/search` per-host brand + ads-suppressed-on-zero-results. Below: token-decrypt hardening + #30/#15/#16._


_Last updated: 2026-05-30 — **#30/#15/#16 verified in a real browser + a token-decrypt hardening fix found while testing.** Drove the local stack (api :3000 + web :3003) as a seeded MEDIA_BUYER and exercised every feature end-to-end: **#15** — Clone → a new `(copy)` DRAFT with offers + ad sets copied and BRAND-NEW redirect ids (confirmed distinct via API); Save preset → "Preset saved" + the row persists; New-from-preset `<select>` → a fresh draft with the saved config; Bulk clone → 3 drafts `(copy 1/2/3)`, each with offers + fresh redirect ids. **#30** — the detail page renders ACTIVE→"Pause campaign" + "Live on Facebook", PAUSED→"Resume campaign" + "Paused"; clicking Resume flipped the UI to ACTIVE with the green "Campaign resumed" note; the local ACTIVE↔PAUSED flip + audit verified via `POST /:id/pause|resume` (200). **#16** — `pnpm --filter @knn/web e2e` → 3/3 smoke pass, 1 `test.fixme` skipped (as designed). **Hardening (the "fix it"):** pausing a campaign whose stored FB token can't be decrypted leaked a raw **`500 ERR_CRYPTO_INVALID_AUTH_TAG`** (reachable in prod via a rotated `TOKEN_ENCRYPTION_KEY` / corrupt ciphertext — and that key is flagged for rotation). Root-caused at the source: `decryptToken` (`@knn/fb/crypto`) now wraps the crypto and throws a typed, token-free **`TokenDecryptError`** (a plain `Error`, NOT an `FbApiError`, so no worker `instanceof FbApiError` logic changes) — upgrading the failure message at all ~18 decrypt call sites. `launch.service.ts` gained a `decryptConnectionToken` helper that maps it to a clean **409 "reconnect the profile in Settings → Facebook"** at all three of its decrypt sites (launch / pause-resume / relaunch). Verified live: the same pause call now returns 409 with the actionable message instead of the raw 500. **Gate:** `@knn/fb` 32 tests (+1 crypto: undecryptable value → `TokenDecryptError`, never a raw crypto code), api 170 tests (+1: pause with a bad stored token → 409 not 500), fb+api typecheck/lint green. Local-only seed scripts deleted. Commit `b88a60f`. **DEPLOYED to staging** (`push-staging.sh api web worker`; dry-run showed zero deletions = box already mirrors HEAD; migrate exited clean, no new migration). **Live-verified:** all containers healthy (api healthy / web / worker freshly recreated, no boot errors; worker `started` + heartbeats; api `Server listening`); `app.staging.rsoc.app/` → 200; the four new routes are registered + gated — `GET /api/campaigns/presets`, `POST /:id/clone`, `POST /:id/bulk-clone`, `POST /:id/pause` each return **401 "Missing bearer token"** (not 404), confirming #15 + #30 shipped. (Full feature click-through was done LOCALLY in-browser — staging has no test-buyer creds to drive auth'd, and pausing a real ACTIVE campaign would stop live spend.) Single deploy = final: the staging test surfaced nothing to fix, so a second identical-commit deploy would be no-op container churn. Below: the three deferred items as shipped._

_Last updated: 2026-05-30 — **The three deferred items shipped: campaign pause/resume (#30) + launcher power features (#15: clone/presets/bulk) + Playwright E2E harness (#16).** **#30** — pause/resume was already fully wired (FB `updateFbCampaignStatus` + `setCampaignActive` + `POST /:id/pause|/resume` + analytics buttons + launch.test); the only gap was the campaign DETAIL page, which now shows ACTIVE→Pause / PAUSED→Resume (owner-scoped). Complete. **#15** (all reuse `toDraft → createCampaign → setOffers`; owner-scoped; each ad gets fresh redirect ids): **CLONE** = `cloneCampaign` / `POST /:id/clone` / list "Clone" → a new editable draft (config + offers copied). **PRESETS** = the pre-existing `launcher_presets` RLS table → new `presets.service` (list / save-from-campaign / apply→new-draft / delete, buyer-scoped) + routes under `/api/campaigns/presets[/:id]` + list-page "Save preset" and a "New from preset" picker. **BULK** = `bulkCloneCampaign` / `POST /:id/bulk-clone` → N (1–20) distinct drafts "(copy 1…N)" + "Bulk clone". **#16** — new `apps/web/playwright.config.ts` + `e2e/smoke.spec.ts` (**3 passing** auth-flow tests: login renders / unauth `/dashboard`→`/login` / signup reachable — web-only since the auth gate treats a missing API session as anon, verified locally with chromium) + `e2e/campaign.spec.ts` (the full "build 2-adset/4-ad → draft → submit" gate scaffolded as `test.fixme` with the API-server + withSystem-seed + wizard-testid plan; its API-level equivalent already passes in `campaigns.test.ts`); a CI `e2e` job; `@playwright/test` dep + `e2e`/`e2e:ui`/`e2e:install` scripts. **Gate:** api **17 campaign tests** (clone ×2, preset ×2, bulk ×2 + existing) + typecheck/lint; web typecheck/lint/build; Playwright 3/3 smoke. No migration (`launcher_presets` already existed). Commits `67bcfb4` (#30+#15), `0626456` (#16). **api + web NOT yet deployed** (the campaign features need the usual deploy auth; #16 is test infra — not deployed, runs in CI). Below: per-host sitemap/robots._

_Last updated: 2026-05-30 — **Per-host `sitemap.xml` + `robots.txt` — faster Google crawl/discovery of new articles (closes the audit's "sitemap/crawl pre-warm" gap).** New `GET /api/public/articles/sitemap?host=` → `listArticleSlugsForHost` (the host's READY article slugs + `lastmod`, host-scoped/tenant-safe, uncapped; +2 tests). Article app: `app/sitemap.xml/route.ts` (per-host `<urlset>` of `https://{host}/a/{slug}`) + `app/robots.txt/route.ts` (replaces the static `robots.ts` — same `Allow:/` `Disallow:/search` plus a per-host `Sitemap:` line for auto-discovery); both dynamic on the request Host, the article list fetched with a 10-min cache. **Framing:** this is crawl/discovery + SEO, NOT an "empty unit" fix — the content-page related-search unit already fills immediately from publisher `terms` (article `relatedSearchTerms` → keywords, passed alongside the RAC); the sitemap just gets Google to crawl fresh URLs sooner (so it can refine content-derived terms + the pages are legitimately known). **No in-app force-crawl exists** (Google deprecated sitemap ping; the Indexing API is content-type-restricted) → operational accelerator = submit the sitemap in Search Console. Gate: api 16 articles tests + typecheck/lint; article typecheck/lint/build (routes show `ƒ /sitemap.xml` + `ƒ /robots.txt`). Commit `f341fad`; **DEPLOYED** (recreated api+article). **Deploy footgun hit + handled:** `git archive | tar -x` does NOT delete files removed in the commit, so the stale `robots.ts` (and `search-ads-unit.tsx`) lingered on the box and Next served the OLD `/robots.txt` — fixed by `rm`-ing them on the box + rebuild — **and then permanently fixed: `deploy/push-staging.sh` (commit `36a3bfd`) mirrors HEAD with `rsync --delete`** (excludes `.env.staging`/`backups/`/`.wrangler`), so removed files now propagate to the box; it cleaned the accumulated cruft on first run. Verified live: `/robots.txt` carries the `Sitemap:` line; `/sitemap.xml` → 9 article URLs with `lastmod`. Below: /search money-page overhaul._

_Last updated: 2026-05-30 — **`/search` money-page overhaul: organic Web results (the compliance fix) + fast inline-during-parse ads + the `ch`→`cid` revenue-attribution fix. All DEPLOYED + live-verified.** Reverse-engineered three live competitor RSOC funnels (rvguide.com / storyimagur.com / creatorrule.com): all three show **organic "Web results"** on their `/search` results page so the AFS ads SUPPLEMENT real results (Search-ads policy answer/7003954: "ads supplement search results, not act as the results themselves"; "number of ads ≤ number of search results") — ours showed ads + a related-search unit only. A real click-through the user captured also proved Google appends **`rsToken` + click telemetry** to our `/search` URL (we ARE on the modern enrolled RSOC build, `referrerAdCreative` passed) — and exposed a bug: Google's telemetry includes **`ch=1`**, which COLLIDED with our `ch` AFS-channel param (Next parsed two `ch` values → array → `str()` dropped it → the `/search` ads fired with NO channel → per-offer AFS revenue silently un-attributed on the page that actually earns). Every competitor uses **`cid`** for the channel for exactly this reason. **Shipped (article + api):** (1) **`ch`→`cid`** — `related-search-unit.tsx` forwards the channel to `/search` as `cid`, `search/page.tsx` reads `sp.cid`; the redirect still emits `ch` on the *article* URL (no Worker change — no Google telemetry on the content page). Verified live: `"channel":"05219"` survives even with `&ch=1` present. (2) **Inline CSA bootstrap** — new SERVER component `search/search-ads.tsx` fires `_googCsa('ads', …)` from a server-rendered inline `<script>` during HTML parse (the official AdSense snippet pattern + XSS-safe `safeJson` for the URL-derived query/rc/channel) instead of the old client `useEffect` that waited for ~100 kB of hydration; + site-wide **preconnects** to Google ad origins via React 19's `preconnect()`/`prefetchDNS()` (`app/resource-hints.tsx`, client comp — raw `<link>` as `<html>` children caused a hydration mismatch, the API does not). (3) **React #418 hydration fix** — diagnosed in local dev (`next dev`, full message): ads.js injects ad `<iframe>`s into `#afscontainer1`/`#relatedsearches1` during parse; `suppressHydrationWarning` does NOT cover injected *child* nodes, so the containers are marked **`dangerouslySetInnerHTML={{__html:''}}`** (externally-managed → React never recurses into or wipes them). (4) **`ignoredPageParams`** removed from the ads call (CSA logs it as unsupported on the results page). (5) **Organic "Web results"** — new `GET /api/public/articles?host=&limit=` → `listArticlesForHost` (articles.service): the host's own READY articles via its offers' article (per-offer variant, else campaign default), newest-first, **host-scoped = tenant-safe**, short public `Cache-Control`. A `<WebResults>` async server component (`search/web-results.tsx`) renders them SERP-style, wrapped in **`<Suspense>`** so it NEVER blocks the ad request (ads flush + fire in the first chunk; results stream in). CSS **`order`** on a flex `.page` sets the visual order **ads → Web results → related searches** (the related container stays flushed-early so ads.js finds it by id — only its visual slot moves; it was a ~915px unit that had pushed the organic results below the fold). Sourced from our OWN articles — the FASTEST option (plain server HTML, no extra Google CSE widget, cached fetch off the critical path) per the "money page, within ms" mandate. **Gate:** api articles module **14/14** (incl. **+6 new `web-results.test.ts`**: host scope, per-offer-variant vs campaign-default, READY-only filter, markdown→snippet, host normalization, unknown host→[]), api typecheck/lint green; article typecheck/lint/build green. **Live-verified on `articles.10linesabout.com/search`:** the API returns 4 real host-scoped articles; the page renders the "Web results" SERP listing **above the fold** with the ads inline-firing (SODAR `adtrafficquality.google` beacon) + `"channel":"05219"`; fresh-tab console **clean** (no #418, no `ignoredPageParams` warning — both confirmed via the same check that caught them on prior deploys). Commits `1b27bf1` (cid + inline ads), `a0c05e4`+`9c2ee08` (#418 hydration), `4ca3ed4` (ignoredPageParams), `8728e68` (Web results + api endpoint + tests), `3644306` (reorder); deploy recreated **api + article**, no migration. **Resolves** the prior session's open compliance gaps on the results page: "ads supplement results" → satisfied (real organic Web results present). **Follow-ups (same session):** content-page related-search unit trimmed **10→6 chips** (matches Google's official RSOC examples — a supplement under the headline/lead, not the page's focus; commit `3700888`); and the **results-page related-search unit was REMOVED** (commit `8d0b8bb`) — once `/search` shows real organic Web results, the related-search widget there is redundant (the Web results ARE the results the ads supplement), so `/search` is now simply header → ads → Web results. The related-search unit lives on the **content page only** (it's the CTA driving clicks to `/search`). `/search` ads + organic Web results are each **capped at 5** (`number:5` on the ad block + `limit=5` on the fetch; commit `ced653c`) — equal caps keep "ads ≤ results" once a host has ≥5 articles (Google may serve fewer ads). Seeded **5 AI articles** onto `articles.10linesabout.com` via a new `packages/db/scripts/seed-domain-articles.ts` (OpenAI generate → content campaign → PAID offer, so `listArticlesForHost` surfaces them; idempotent per org+query; commit `e984f0a`) — the demo host now has 9 articles, and `/search` is verified showing **5 Web results** under the `number:5` ads. **Note:** a manual test URL may show no-fill ads (no `rsToken` provenance), though high-commercial queries do fill live (verified — real Policybazaar/HDFC ads on `auto insurance quotes`); real FB→cloak→article→click traffic carries `rsToken`. Below: article AFS/RSOC compliance + mobile-fold._
