...
export async function internalRoutes(app: FastifyInstance): Promise<void> {
  ...
  // Force-relaunch: pause the old FB campaign + re-create with current config (e.g. a
  // corrected REDIRECT_DOMAIN). Token-guarded; runs as the campaign's buyer.
  app.post<{ Params: { id: string } }>('/relaunch/:id', async (req, reply) => {
    const header = req.headers['x-internal-token'];
    const tok = Array.isArray(header) ? header[0] : header;
    if (!env.INTERNAL_API_TOKEN || tok !== env.INTERNAL_API_TOKEN) {
      return reply.code(401).send({ error: 'Unauthorized' });
    ...
    try {
      ...
      if (!campaign) return reply.code(404).send({ error: 'Campaign not found' });
      const auth: AuthContext = { userId: campaign.buyerId, orgId: campaign.orgId, role: ROLES.MEDIA_BUYER, status: USER_STATUS.ACTIVE };
      return reply.send(await relaunchCampaign(auth, req.params.id));
    } catch (err) {
      return handleRouteError(err, reply);
    }
  });
  ...
  // Manager assets appear without a manual "Sync". Called by the worker's connection-sync cron.
  app.post('/sync-connections', async (req, reply) => {
    const header = req.headers['x-internal-token'];
    const tok = Array.isArray(header) ? header[0] : header;
    if (!env.INTERNAL_API_TOKEN || tok !== env.INTERNAL_API_TOKEN) {
      return reply.code(401).send({ error: 'Unauthorized' });
