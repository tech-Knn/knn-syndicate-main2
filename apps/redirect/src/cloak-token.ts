/**
 * Cloak token — a short-lived signed blob that proves a click came from THIS redirect, and carries
 ...
 * Uses Web Crypto (`crypto.subtle`) so the SAME code runs on the Cloudflare Worker (mint) and the
 * Next article server (verify). ⚠️ This file is duplicated verbatim at
 * `apps/article/app/_afs/cloak-token.ts` — keep the two identical (round-trip tests guard both).
 */
