...
worker.get('/go/:id', async (c) => {
  ...
  // On a funnel-bound (paid + active) click, log txid → {redirectId, fbclid, ts, fbp,
  // clientIp} to KV so the later conversion beacon can resolve the ad's pixel + the
  // buyer's token, the fbclid for attribution, the CLICK timestamp (feeds `fbc`'s middle
  // field — must be the FB-ad-click time, not the conversion time, or Facebook rejects
  // the match), a synthesized `fbp` browser id (pure-S2S: no in-browser Meta pixel to
  // set `_fbp`, so we mint one here and reuse it across every funnel event for this
  // click — Facebook accepts the value regardless of whether it came from a cookie or
  // the server, only the format and stability-per-visitor matter for EMQ), and the
  // visitor's real IP as seen by Cloudflare at click time (CF-Connecting-IP is the
  // authoritative source — same IP Facebook saw when issuing the fbclid, so it's the
  // best match signal; beacon-time req.ip goes through Caddy and can end up as a shared
  // proxy IP, triggering the "IP addresses associated with multiple users" warning).
  // Fire-and-forget (waitUntil) — never blocks the 302.
